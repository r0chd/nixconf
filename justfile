...
home-build:
    @nix build ".#homeConfigurations.$(whoami)@$(hostname).config.home.activationPackage" --log-format internal-json --verbose --out-link /tmp/nh-homeEBVKhH/result |& nom --json

home-switch:
    @/tmp/nh-homeEBVKhH/result$(cat /etc/specialisation > /dev/null 2>&1 && echo /specialisation/$(cat /etc/specialisation)/activate || echo /activate) test
    @nvd diff {{HOME_ACTIVATION_PATH}} /tmp/nh-homeEBVKhH/result/specialisation/niri
