    ...
    home-manager.users."${username}" = {
      ...
      wayland.windowManager.hyprland = {
        ...
        settings = {
          ...
          env =
            [ ]
            ++ (lib.optional (config.home-manager.users.${username}.cursor.enable)
              "HYPRCURSOR_SIZE,${toString config.home-manager.users.${username}.cursor.size}"
            );
          ...
          bind =
            ...
            ++ (lib.optional config.audio.enable ", XF86AudioRaiseVolume, exec, pamixer -i 5")
            ++ (lib.optional config.audio.enable ", XF86AudioLowerVolume, exec, pamixer -d 5")
            ++ (lib.optional (config.home-manager.users.${username}.terminal.enable
            ) "$mainMod SHIFT, RETURN, exec, ${config.home-manager.users.${username}.terminal.program}")
            ++ (lib.optional config.home-manager.users.${username}.seto.enable
              '', Print, exec, grim -g "$(seto -r)" - | wl-copy -t image/png''
            )
            ++ (lib.optional (
              config.home-manager.users.${username}.seto.enable && ydotool.enable
            ) "$mainMod, G, exec, click")
            ++ (lib.optional (config.home-manager.users.${username}.launcher.enable)
              "$mainMod, S, exec, ${config.home-manager.users.${username}.launcher.program}"
            );
