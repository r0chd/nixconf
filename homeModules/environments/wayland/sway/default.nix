    ...
    home-manager.users."${username}" = {
      wayland.windowManager.sway = {
        ...
        config = rec {
          bars =
            [ ]
            ++ (lib.optional (config.home-manager.users.${username}.statusBar.enable) {
              command = "${config.home-manager.users.${username}.statusBar.program}";
            });
          modifier = "Mod1";
          gaps.outer = 7;
          ...
          input."9011:26214:ydotoold_virtual_device" = lib.mkIf config.ydotool.enable {
            "accel_profile" = "flat";
          };

          startup =
            [ ]
            ++ lib.optional (config.home-manager.users.${username}.terminal.enable) {
              command = "sway workspace 1; ${config.home-manager.users.${username}.terminal.program}";
            }
            ++ lib.optional (config.home-manager.users.${username}.wallpaper.enable) {
              command = "${config.home-manager.users.${username}.wallpaper.program} ${
          ...
          keybindings =
            ...
            // lib.optionalAttrs config.home-manager.users.${username}.seto.enable {
              "Print" = ''exec grim -g "$(seto -r)" - | wl-copy -t image/png'';
            }
            // lib.optionalAttrs (config.home-manager.users.${username}.seto.enable) {
              "${modifier}+g" = ''exec "ydotool mousemove -a $(seto -f "%x %y") && ydotool click 0xC0"'';
            }
            // lib.optionalAttrs (config.home-manager.users.${username}.terminal.enable) {
              "${modifier}+Shift+Return" = "exec ${config.home-manager.users.${username}.terminal.program}";
            }
            // lib.optionalAttrs (config.home-manager.users.${username}.launcher.enable) {
              "${modifier}+S" = "exec ${config.home-manager.users.${username}.launcher.program}";
            };
        };
      };
