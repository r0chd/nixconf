...
in
{
  ...
  config = lib.mkMerge [
    ...
    (lib.mkIf (builtins.hasAttr "manifests" cfg && builtins.length (lib.attrNames cfg.manifests) > 0) {
      systemd.services = lib.listToAttrs (
        lib.flatten (
          lib.mapAttrsToList (manifestName: manifest:
            lib.optional (builtins.hasAttr "content" manifest && manifest.content != null) {
              name = "k3s-manifest-${manifestName}";
              value = {
                enable = true;
                description = "k3s manifest for ${manifestName}";
                after = [ "k3s.service" ];
                requires = [ "k3s.service" ];
                wantedBy = [ "multi-user.target" ];
                ...
                script = ''
                  ${pkgs.kubectl}/bin/kubectl apply -f - <<'EOF'
                  ${lib.concatMapStringsSep "\n---\n" (manifestItem: "${builtins.toJSON manifestItem}") (if manifest.content != null then manifest.content else [])}
                  EOF
                '';
              };
            }
