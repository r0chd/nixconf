{ lib, ... }:
{
  services.k3s.manifests.grafana-cloudnative-pg-dashboard.content = [
    {
      apiVersion = "v1";
      kind = "ConfigMap";
      metadata = {
      ...
      data."cloudnative-pg.json" = lib.generators.toJSON { } {
        __elements = { };
        __inputs = [
          {
            description = "";
        ...
        panels = [
          ...
          }
          {
            ...
            description = ''
              Computes the time since the last known WAL archival in the primary.
            ...
            targets = [
              {
                ...
                exemplar = false;
                expr = ''
                  max((1 - cnpg_pg_replication_in_recovery{namespace=~"$namespace",pod=~"$instances"}) * (time() - timestamp(cnpg_pg_stat_archiver_seconds_since_last_archival{namespace=~"$namespace",pod=~"$instances"}) +
          ...
          }
          {
            ...
            description = ''
              This metric depends on exporting the: `topology.kubernetes.io/zone` label through kube-state-metrics (not enabled by default). Can be added by changing its configuration with:
