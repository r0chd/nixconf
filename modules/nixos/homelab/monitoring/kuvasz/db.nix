{ config, lib, ... }:
let
  cfg = config.homelab.monitoring.kuvasz.db;
  inherit (lib) types;
in
{
  options.homelab.monitoring.kuvasz.db = {
  ...
  config = lib.mkIf (config.homelab.enable && config.homelab.monitoring.kuvasz.enable) {
    services.k3s = {
      manifests.cnpg-databases.content = [
        {
          ...
          kind = "NetworkPolicy";
          metadata = {
            name = "kuvasz-db-allow-app";
            namespace = "monitoring";
          };
          spec = {
            podSelector.matchLabels."cnpg.io/cluster" = "kuvasz-db";
            ingress = [
              {
                from = [
                  {
                    podSelector.matchLabels."app.kubernetes.io/name" = "kuvasz";
                  }
                ];
                ports = [
                  {
...

        {
          ...
          kind = "NetworkPolicy";
          metadata = {
            name = "kuvasz-db-allow-monitoring";
            namespace = "monitoring";
          };
          spec = {
            podSelector.matchLabels."cnpg.io/cluster" = "kuvasz-db";
...

        {
          ...
          kind = "NetworkPolicy";
          metadata = {
            name = "kuvasz-db-allow-inter-node";
            namespace = "monitoring";
          };
          spec = {
            podSelector.matchLabels."cnpg.io/cluster" = "kuvasz-db";
...

        {
          ...
          // lib.optionalAttrs cfg.backup.enable {
            backup = {
              barmanObjectStore = {
                destinationPath = "s3://kuvasz-backup";
                endpointURL = "https://s3.${config.homelab.garage.ingressHost}";
                s3Credentials = {
                  accessKeyId = {
                    name = "kuvasz-backup-credentials";
                    key = "access-key-id";
                  };
                  secretAccessKey = {
                    name = "kuvasz-backup-credentials";
