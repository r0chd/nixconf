...
in
{
  ...
  sops = {
      ...
      |> builtins.concatMap (user: [
        {
          ...
          value = {
            neededForUsers = true;
            sopsFile = "${inputs.self}/hosts/${config.networking.hostName}/users/${user}/secrets/secrets.yaml";
          };
        }
        {
          name = "${user}/ssh";
    ...
    defaultSopsFile = "${inputs.self}/hosts/${config.networking.hostName}/secrets/secrets.yaml";
